...
- 詳細検索モード: グループ式ビルダーで `(A AND NOT B) OR (C AND D) OR E` のような複合論理式を構築可能——グループ内のチップをクリックするとNOT（除外）を切替
- 検索条件の保存: ツールバーのブックマークメニューから現在の条件に名前を付けて保存し、後から編集可能な形で呼び出せます（同名保存は上書き）
- タグ名・ファイル名それぞれのインクリメンタル絞り込み
- 検索結果のファイルを選択してスペースキーでQuick Lookプレビュー（上下カーソルキーでプレビュー対象を切替）
- タグ編集パレット（ツールバーボタン・Cmd+T・Quick Look表示中はTキー）: プレビュー中のファイルへのタグ付与/解除、色付き新規タグの作成——ファイル上の既存タグの色は保持されます
